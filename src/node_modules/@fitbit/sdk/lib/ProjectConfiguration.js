...
(function (Locales) {
    ...
    Locales["cs-CZ"] = "Czech";
    Locales["nb-NO"] = "Norwegian (Bokm\u00E5l)";
...
(function (Permission) {
    ...
    Permission["FITBIT_INTERNAL"] = "fitbit_internal";
...
function normalizeProjectConfig(config, defaults) {
    ...
    else if (!Array.isArray(mergedConfig.appClusterID) &&
        config.appClusterID !== undefined) {
        throw new TypeError(`App Cluster ID field has unknown type ${typeof config.appClusterID}`);
    }
    return mergedConfig;
}
...
function validateClusterID(clusterID) {
    if (clusterID.length < 1 || clusterID.length > exports.MAX_LENGTH_APP_CLUSTER_ID) {
...
function validateStorageGroup(config) {
    ...
    if (hasRequestedPermission) {
        if (config.appClusterID === undefined) {
            ...
            diagnostics.pushFatalError(`Only a single App Cluster ID may be declared`);
        }
        else {
            for (const clusterID of config.appClusterID) {
                const validationError = validateClusterID(clusterID);
                if (validationError) {
                    diagnostics.pushFatalError(`App Cluster ID '${clusterID}' ${validationError}`);
                }
            }
        }
...
function validateHeapSize(config) {
    const diagnostics = new DiagnosticList_1.default();
    const isFitbitInternal = (config.requestedPermissions || []).includes(Permission.FITBIT_INTERNAL);
    if (typeof config.heapSize !== 'undefined' &&
        (!Number.isInteger(config.heapSize) || config.heapSize < MIN_HEAP_SIZE)) {
        diagnostics.pushFatalError(`Heap size must be an integer value greater than or equal to ${MIN_HEAP_SIZE}`);
    }
    if (!isFitbitInternal &&
        typeof config.heapSize !== 'undefined' &&
        (!Number.isInteger(config.heapSize) || config.heapSize > MAX_HEAP_SIZE)) {
        diagnostics.pushFatalError(`Heap size must be an integer value less than or equal to ${MAX_HEAP_SIZE}`);
    }
    return diagnostics;
}
exports.validateHeapSize = validateHeapSize;
function validateTileComponentAppType(config) {
    const diagnostics = new DiagnosticList_1.default();
    if (config.appType !== AppType.APP && config.tiles !== undefined) {
        diagnostics.pushWarning('Tiles available only for APPS. Skipping tile configuration!');
    }
    return diagnostics;
}
exports.validateTileComponentAppType = validateTileComponentAppType;
function validateTileBuildTarget(tile, config) {
    ...
    return new DiagnosticList_1.default();
}
exports.validateTileBuildTarget = validateTileBuildTarget;
function validateTileUUID(tile, config) {
    var _a;
    const diagnostic = new DiagnosticList_1.default();
    if (tile.uuid === undefined) {
        diagnostic.pushFatalError("Tile UUID can't be undefined");
    }
    if (!validator_1.default.isUUID(String(tile.uuid))) {
        diagnostic.pushFatalError('Tile UUID must be a valid UUID');
    }
    const knownUUID = (_a = config.tiles) === null || _a === void 0 ? void 0 : _a.map((tile) => tile.uuid);
    if ((knownUUID === null || knownUUID === void 0 ? void 0 : knownUUID.indexOf(tile.uuid)) !== (knownUUID === null || knownUUID === void 0 ? void 0 : knownUUID.lastIndexOf(tile.uuid))) {
        diagnostic.pushFatalError("Can't have duplicate UUIDs between tiles");
    }
    return diagnostic;
}
exports.validateTileUUID = validateTileUUID;
function validateTileNativeComponent(hasNativeComponents) {
    const diagnostic = new DiagnosticList_1.default();
    if (!hasNativeComponents) {
        diagnostic.pushWarning('Tiles available only for native components. Skipping tile configuration!');
    }
    return diagnostic;
}
exports.validateTileNativeComponent = validateTileNativeComponent;
function validateTileName(tile, config) {
    const diagnostic = new DiagnosticList_1.default();
...
exports.validateTileName = validateTileName;
function validate(config, options) {
    ...
    if (config.appType === AppType.APP) {
        ...
        [validateTileBuildTarget, validateTileUUID, validateTileName].forEach((validator) => {
            var _a;
            (_a = appConfig.tiles) === null || _a === void 0 ? void 0 : _a.forEach((tile) => diagnostics.extend(validator(tile, appConfig)));
        });
