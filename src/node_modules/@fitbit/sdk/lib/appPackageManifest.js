...
class AppPackageManifestTransform extends stream_1.Transform {
    ...
    populateTileData() {
        var _a;
        if (this.projectConfig.appType === ProjectConfiguration_1.AppType.APP) {
            const appConfig = this.projectConfig;
            if (appConfig.tiles !== undefined && this.hasNative) {
                const watchComponents = Object.keys((_a = this.components.watch) !== null && _a !== void 0 ? _a : []);
                this.components.tiles = appConfig.tiles.map((tile) => ({
                    name: tile.name,
                    id: tile.uuid,
                    platforms: tile.buildTargets !== undefined
                        ? tile.buildTargets.filter((target) => watchComponents.includes(target))
                        : watchComponents,
                }));
                this.components.tiles = this.components.tiles.filter((tile) => tile.platforms.length > 0);
